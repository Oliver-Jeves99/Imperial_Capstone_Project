...
settings = {
    "f1": dict(acq="ucb", beta=6.5, frac_local=0.20, top_k=4, local_sigma=0.20,
               boundary_penalty_weight=0.12, boundary_eps=0.03),

    "f2": dict(acq="ei", xi=0.03, frac_local=0.85, top_k=7, local_sigma=0.09,
               max_step=0.18, min_dist_threshold=0.06,
               boundary_penalty_weight=0.06, boundary_eps=0.03),

    "f3": dict(acq="ucb", beta=3.0, frac_local=0.90, top_k=7, local_sigma=0.12,
               interior_eps=0.02, boundary_penalty_weight=0.20, boundary_eps=0.05,
               max_step=0.22, min_dist_threshold=0.06),

    "f4": dict(acq="ei", xi=0.02, frac_local=0.95, top_k=8, local_sigma=0.045,
               max_step=0.08, min_dist_threshold=0.06,
               interpret=True, interpret_repeats=10,
               boundary_penalty_weight=0.05, boundary_eps=0.03),

    "f5": dict(acq="ei", xi=0.04,
               frac_local=1.00, top_k=5, local_sigma=0.07,
               max_step=0.10, min_dist_threshold=0.08,
               interpret=True, interpret_repeats=10,
               boundary_penalty_weight=0.00, boundary_eps=0.03),

    "f6": dict(acq="ucb", beta=3.5, frac_local=0.70, top_k=7, local_sigma=0.12,
               interior_eps=0.01,
               boundary_penalty_weight=0.10, boundary_eps=0.04,
               max_step=0.25, min_dist_threshold=0.06),

    "f7": dict(acq="ei", xi=0.02, frac_local=0.95, top_k=8, local_sigma=0.06,
               max_step=0.12, min_dist_threshold=0.07,
               boundary_penalty_weight=0.05, boundary_eps=0.03),

    "f8": dict(acq="ei", xi=0.02, frac_local=0.90, top_k=8, local_sigma=0.08,
               max_step=0.20, min_dist_threshold=0.06,
               boundary_penalty_weight=0.05, boundary_eps=0.03),
}
...
x1 = propose_next_point_hybrid(f1_inputs, f1_outputs, n_candidates=n_cand_default, seed=WEEK_SEED+1, **settings["f1"])
x2 = propose_next_point_hybrid(f2_inputs, f2_outputs, n_candidates=n_cand_default, seed=WEEK_SEED+2, **settings["f2"])
x3 = propose_next_point_hybrid(f3_inputs, f3_outputs, n_candidates=n_cand_default, seed=WEEK_SEED+3, **settings["f3"])
x4 = propose_next_point_hybrid(f4_inputs, f4_outputs, n_candidates=n_cand_default, seed=WEEK_SEED+4, **settings["f4"])
x5 = propose_next_point_hybrid(f5_inputs, f5_outputs, n_candidates=220_000, seed=WEEK_SEED+5, **settings["f5"])
x6 = propose_next_point_hybrid(f6_inputs, f6_outputs, n_candidates=n_cand_high_d, seed=WEEK_SEED+6, **settings["f6"])
x7 = propose_next_point_hybrid(f7_inputs, f7_outputs, n_candidates=n_cand_high_d, seed=WEEK_SEED+7, **settings["f7"])
x8 = propose_next_point_hybrid(f8_inputs, f8_outputs, n_candidates=n_cand_high_d, seed=WEEK_SEED+8, **settings["f8"])

print("\n--- Week 7 proposed queries (submit these) ---")
print("Function 1:", fmt_point(x1))
print("Function 2:", fmt_point(x2), "(EI)")
print("Function 3:", fmt_point(x3), "(interior-constrained)")
print("Function 4:", fmt_point(x4), "(EI, tight exploit)")
print("Function 5:", fmt_point(x5), "(EI, LOCKED LOCAL near best)")
print("Function 6:", fmt_point(x6))
print("Function 7:", fmt_point(x7), "(EI, tight exploit)")
print("Function 8:", fmt_point(x8), "(EI)")
