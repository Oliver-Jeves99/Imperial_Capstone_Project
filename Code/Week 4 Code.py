...
# ------------------- Append Week 3 observation (NEW outputs you just posted) -------------------
...
settings = {
    # F1 flat -> explore
    "f1": dict(acq="ucb", beta=5.5, frac_local=0.15, top_k=3, local_sigma=0.16),

    # F2 still good but drifted down -> mostly exploit, keep some exploration
    "f2": dict(acq="ei", xi=0.02, frac_local=0.75, top_k=6, local_sigma=0.09),

    # F3 worsened -> hybrid: local around best + global search (UCB)
    "f3": dict(acq="ucb", beta=4.0, frac_local=0.60, top_k=6, local_sigma=0.12),

    # F4 ridge: positive again -> careful EI trust-region refinement
    "f4": dict(acq="ei", xi=0.02, frac_local=0.80, top_k=6, local_sigma=0.030,
               interpret=True, interpret_repeats=10),

    # F5 strong basin -> EI exploit + diversity constraint
    "f5": dict(acq="ei", xi=0.01, frac_local=0.90, top_k=7, local_sigma=0.06,
               interpret=True, min_dist_threshold=0.08),

    # F6 still regressing -> explore more
    "f6": dict(acq="ucb", beta=3.5, frac_local=0.40, top_k=5, local_sigma=0.14),

    # F7 improved -> refine locally around best
    "f7": dict(acq="ei", xi=0.02, frac_local=0.80, top_k=7, local_sigma=0.10),

    # F8 still high -> exploit, but keep some global candidates to avoid boundary traps
    "f8": dict(acq="ei", xi=0.02, frac_local=0.75, top_k=8, local_sigma=0.08),
}

n_cand_default = 220_000
n_cand_high_d = 280_000
