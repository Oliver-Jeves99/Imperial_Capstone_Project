...
f1_inputs = np.vstack([f1_inputs, [0.764663, 0.905273]])
f1_outputs = np.append(f1_outputs, 8.015849166654494e-68)

f2_inputs = np.vstack([f2_inputs, [0.692725, 0.278929]])
f2_outputs = np.append(f2_outputs, 0.5605145847513243)

f3_inputs = np.vstack([f3_inputs, [0.369284, 0.416431, 0.301011]])
f3_outputs = np.append(f3_outputs, -0.07720936803871446)

f4_inputs = np.vstack([f4_inputs, [0.455240, 0.446060, 0.268139, 0.441884]])
f4_outputs = np.append(f4_outputs, -1.7128668059186611)

f5_inputs = np.vstack([f5_inputs, [0.231670, 0.850028, 1.000000, 0.889556]])
f5_outputs = np.append(f5_outputs, 1980.5547253042491)

f6_inputs = np.vstack([f6_inputs, [0.433148, 0.596539, 0.193715, 0.950096, 0.025464]])
f6_outputs = np.append(f6_outputs, -0.9172054373685039)

f7_inputs = np.vstack([f7_inputs, [0.000000, 0.381704, 0.139019, 0.294245, 0.465160, 0.557858]])
f7_outputs = np.append(f7_outputs, 1.161948203881051)

f8_inputs = np.vstack([f8_inputs, [0.197772, 0.083170, 0.101703, 0.000000, 0.971840, 0.425134, 0.239160, 0.541693]])
f8_outputs = np.append(f8_outputs, 9.9277364699241)



settings = {
    "f1": dict(acq="ucb", beta=5.0, frac_local=0.15, top_k=3, local_sigma=0.14),

    "f2": dict(acq="ei",  xi=0.01, frac_local=0.85, top_k=6, local_sigma=0.08),

    "f3": dict(acq="ucb", beta=3.5, frac_local=0.50, top_k=5, local_sigma=0.11),

    "f4": dict(acq="ei",  xi=0.02, frac_local=0.80, top_k=6, local_sigma=0.035,
               interpret=True, interpret_repeats=10),

    "f5": dict(acq="ei",  xi=0.02, frac_local=0.90, top_k=7, local_sigma=0.07,
               interpret=True, min_dist_threshold=0.08),

    "f6": dict(acq="ucb", beta=3.0, frac_local=0.45, top_k=5, local_sigma=0.12),

    "f7": dict(acq="ucb", beta=2.3, frac_local=0.60, top_k=6, local_sigma=0.10),

    "f8": dict(acq="ei",  xi=0.01, frac_local=0.85, top_k=8, local_sigma=0.07),
}

n_cand_default = 200_000
n_cand_high_d = 260_000
...
print("\n--- Week 3 proposed queries ---")
print("Next query for Function 1:", submission1)
print("Next query for Function 2:", submission2, "(EI)")
print("Next query for Function 3:", submission3)
print("Next query for Function 4:", submission4, "(EI, smaller step)")
print("Next query for Function 5:", submission5, "(EI + min-dist)")
print("Next query for Function 6:", submission6)
print("Next query for Function 7:", submission7)
print("Next query for Function 8:", submission8, "(EI)")
