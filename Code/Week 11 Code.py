...
settings = {
    "f1": dict(
        acq="ucb", beta=10.0,
        frac_local=0.55, top_k=6, local_sigma=0.18,
        max_step=0.35,
    ...
    "f2": dict(
        acq="ei", xi=0.02,
        frac_local=0.60, top_k=8, local_sigma=0.09,
        max_step=0.22, min_dist_threshold=0.06,
        boundary_penalty_weight=0.05, boundary_eps=0.03,
        interior_eps=0.005,
    ),

    "f3": dict(
        acq="ei", xi=0.01,
        frac_local=0.95, top_k=10, local_sigma=0.05,
        max_step=0.12, min_dist_threshold=0.06,
        interior_eps=0.01,
        boundary_penalty_weight=0.10, boundary_eps=0.04,
    ),

    "f4": dict(
        acq="ei", xi=0.02,
        frac_local=0.88, top_k=12, local_sigma=0.06,
        max_step=0.14, min_dist_threshold=0.05,
        interpret=True, interpret_repeats=10,
        boundary_penalty_weight=0.08, boundary_eps=0.03,
        interior_eps=0.005,
    ),

    "f5": dict(
        acq="ei", xi=0.005,
        frac_local=1.00, top_k=2, local_sigma=0.02,
        max_step=0.05, min_dist_threshold=0.09,
        interpret=True, interpret_repeats=12,
        boundary_penalty_weight=0.00, boundary_eps=0.06,
        interior_eps=0.00,
    ),

    "f6": dict(
        acq="ei", xi=0.01,
        frac_local=0.90, top_k=12, local_sigma=0.06,
        max_step=0.12, min_dist_threshold=0.08,
        interior_eps=0.01,
        boundary_penalty_weight=0.06, boundary_eps=0.04,
    ),

    "f7": dict(
        acq="ei", xi=0.015,
        frac_local=0.95, top_k=12, local_sigma=0.05,
        max_step=0.10, min_dist_threshold=0.06,
        interior_eps=0.01,
        boundary_penalty_weight=0.05, boundary_eps=0.03,
    ),
    ...
    "f8": dict(
        acq="ei", xi=0.003,
        frac_local=1.00, top_k=14, local_sigma=0.03,
        max_step=0.08, min_dist_threshold=0.10,
        interior_eps=0.00,
        boundary_penalty_weight=0.00, boundary_eps=0.03,
    ),
}

n_cand_default = 280_000
n_cand_high_d = 360_000

x1 = propose_next_point_hybrid(f1_inputs, f1_outputs, n_candidates=320_000, **settings["f1"])
x2 = propose_next_point_hybrid(f2_inputs, f2_outputs, n_candidates=320_000, **settings["f2"])
...
x5 = propose_next_point_hybrid(f5_inputs, f5_outputs, n_candidates=240_000, **settings["f5"])
