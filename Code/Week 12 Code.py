...
f1_inputs = np.vstack([f1_inputs, [0.125294, 0.030013]]); f1_outputs = np.append(f1_outputs, -2.81656234392154e-168)
f2_inputs = np.vstack([f2_inputs, [0.694638, 0.587177]]); f2_outputs = np.append(f2_outputs, 0.7253510439472333)
f3_inputs = np.vstack([f3_inputs, [0.278027, 0.236401, 0.401520]]); f3_outputs = np.append(f3_outputs, -0.03737982294313748)
f4_inputs = np.vstack([f4_inputs, [0.442869, 0.378480, 0.416209, 0.440244]]); f4_outputs = np.append(f4_outputs, 0.12583909621144018)
f5_inputs = np.vstack([f5_inputs, [0.454623, 0.949402, 0.863102, 0.381059]]); f5_outputs = np.append(f5_outputs, 652.5834920290789)
f6_inputs = np.vstack([f6_inputs, [0.452827, 0.357699, 0.542137, 0.744860, 0.144727]]); f6_outputs = np.append(f6_outputs, -0.2639065306540293)
f7_inputs = np.vstack([f7_inputs, [0.010000, 0.242657, 0.289943, 0.136976, 0.319597, 0.769534]]); f7_outputs = np.append(f7_outputs, 1.9192299263973671)
f8_inputs = np.vstack([f8_inputs, [0.146720, 0.116201, 0.144555, 0.169959, 0.968782, 0.815372, 0.258301, 0.581701]]); f8_outputs = np.append(f8_outputs, 9.8729235263549)

f1_inputs = np.vstack([f1_inputs, [0.951031, 0.931875]]); f1_outputs = np.append(f1_outputs, 1.0734049758120986e-136)
f2_inputs = np.vstack([f2_inputs, [0.696542, 0.665386]]); f2_outputs = np.append(f2_outputs, 0.6155886898637561)
f3_inputs = np.vstack([f3_inputs, [0.089591, 0.795721, 0.406660]]); f3_outputs = np.append(f3_outputs, -0.024595612716559724)
f4_inputs = np.vstack([f4_inputs, [0.407511, 0.403190, 0.423775, 0.381226]]); f4_outputs = np.append(f4_outputs, 0.4933031587814338)
f5_inputs = np.vstack([f5_inputs, [0.142222, 0.978495, 0.931344, 0.994133]]); f5_outputs = np.append(f5_outputs, 3297.7217368138495)
f6_inputs = np.vstack([f6_inputs, [0.488574, 0.383693, 0.641938, 0.703241, 0.051738]]); f6_outputs = np.append(f6_outputs, -0.1981998455655482)
f7_inputs = np.vstack([f7_inputs, [0.031915, 0.241729, 0.259760, 0.461666, 0.312240, 0.772654]]); f7_outputs = np.append(f7_outputs, 1.753969756230923)
f8_inputs = np.vstack([f8_inputs, [0.095474, 0.191850, 0.150169, 0.035276, 0.999999, 0.532375, 0.177350, 0.551759]]); f8_outputs = np.append(f8_outputs, 9.9615189412554)

f1_inputs = np.vstack([f1_inputs, [0.631710, 0.551193]]); f1_outputs = np.append(f1_outputs, -0.00013581978752714974)
f2_inputs = np.vstack([f2_inputs, [0.696567, 0.203520]]); f2_outputs = np.append(f2_outputs, 0.5831373937363569)
f3_inputs = np.vstack([f3_inputs, [0.224464, 0.938618, 0.040652]]); f3_outputs = np.append(f3_outputs, -0.07334231547486865)
f4_inputs = np.vstack([f4_inputs, [0.402255, 0.445063, 0.399128, 0.401279]]); f4_outputs = np.append(f4_outputs, -0.032277349217874285)
f5_inputs = np.vstack([f5_inputs, [0.264913, 0.927041, 0.986518, 0.973204]]); f5_outputs = np.append(f5_outputs, 3127.434273947226)
f6_inputs = np.vstack([f6_inputs, [0.474984, 0.339384, 0.616963, 0.767995, 0.041047]]); f6_outputs = np.append(f6_outputs, -0.11927216849962041)
f7_inputs = np.vstack([f7_inputs, [0.035275, 0.249307, 0.282225, 0.076968, 0.391412, 0.791873]]); f7_outputs = np.append(f7_outputs, 1.386701920580902)
f8_inputs = np.vstack([f8_inputs, [0.090766, 0.109030, 0.425401, 0.405419, 0.187138, 0.844990, 0.542531, 0.720667]]); f8_outputs = np.append(f8_outputs, 9.1281977675911)

for i, (Xin, yout) in enumerate([
    (f1_inputs, f1_outputs), (f2_inputs, f2_outputs), (f3_inputs, f3_outputs), (f4_inputs, f4_outputs),
    (f5_inputs, f5_outputs), (f6_inputs, f6_outputs), (f7_inputs, f7_outputs), (f8_inputs, f8_outputs),
], start=1):
    assert len(Xin) == len(yout), f"Length mismatch in function {i}: X={len(Xin)} y={len(yout)}"
print("✅ Data lengths OK for all functions.")

settings = {
    "f1": dict(
        acq="ucb", beta=8.0,
        frac_local=0.30, top_k=5, local_sigma=0.14,
        min_dist_threshold=0.08,
        boundary_penalty_weight=0.02, boundary_eps=0.03,
        interior_eps=0.00,
    ),

    "f2": dict(
        acq="ei", xi=0.015,
        frac_local=0.80, top_k=8, local_sigma=0.055,
        max_step=0.12, min_dist_threshold=0.04,
        boundary_penalty_weight=0.03, boundary_eps=0.03,
    ),

    "f3": dict(
        acq="ei", xi=0.02,
        frac_local=0.88, top_k=8, local_sigma=0.06,
        max_step=0.12, min_dist_threshold=0.04,
        interior_eps=0.005,
        boundary_penalty_weight=0.05, boundary_eps=0.04,
    ),

    "f4": dict(
        acq="ei", xi=0.01,
        frac_local=1.00, top_k=5, local_sigma=0.02,
        max_step=0.04, min_dist_threshold=0.02,
        interpret=True, interpret_repeats=10,
        boundary_penalty_weight=0.08, boundary_eps=0.04,
        interior_eps=0.00,
    ),

    "f5": dict(
        acq="ei", xi=0.008,
        frac_local=1.00, top_k=3, local_sigma=0.018,
        max_step=0.04, min_dist_threshold=0.03,
        interpret=True, interpret_repeats=10,
        boundary_penalty_weight=0.00, boundary_eps=0.05,
        interior_eps=0.00,
    ),

    "f6": dict(
        acq="ei", xi=0.015,
        frac_local=0.92, top_k=8, local_sigma=0.05,
        max_step=0.10, min_dist_threshold=0.05,
        interior_eps=0.005,
        boundary_penalty_weight=0.03, boundary_eps=0.04,
    ),

    "f7": dict(
        acq="ei", xi=0.02,
        frac_local=0.90, top_k=8, local_sigma=0.06,
        max_step=0.11, min_dist_threshold=0.05,
        interior_eps=0.005,
        boundary_penalty_weight=0.03, boundary_eps=0.03,
    ),
    ...
    "f8": dict(
        acq="ei", xi=0.008,
        frac_local=1.00, top_k=5, local_sigma=0.02,
        max_step=0.05, min_dist_threshold=0.03,
        boundary_penalty_weight=0.00, boundary_eps=0.03,
        interior_eps=0.00,
    ),
}
...
x4 = propose_next_point_hybrid(f4_inputs, f4_outputs, n_candidates=220_000, **settings["f4"])
...
x8 = propose_next_point_hybrid(f8_inputs, f8_outputs, n_candidates=220_000, **settings["f8"])

print("\n--- Revised Week 12 proposed queries (submit these) ---")
print("Function 1:", fmt_point(x1))
print("Function 2:", fmt_point(x2), "(EI)")
print("Function 3:", fmt_point(x3), "(EI)")
print("Function 4:", fmt_point(x4), "(EI + interpret, tightened)")
print("Function 5:", fmt_point(x5), "(EI, locked very local)")
print("Function 6:", fmt_point(x6), "(EI)")
print("Function 7:", fmt_point(x7), "(EI)")
print("Function 8:", fmt_point(x8), "(EI, tightened local)")
